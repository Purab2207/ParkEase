...
export default function UPIPaymentModal({ isOpen, onClose, onSuccess, amount, bayCode, lotName, onCreateBooking }) {
  ...
  const handleAppSelect = async (app) => {
    ...
    setStage('processing');
    setCountdown(5);
    // Create the actual booking during "processing"
    try {
      await onCreateBooking?.();
    } catch (err) {
      // If booking fails, show error and go back
      setStage('select');
      return;
    }
  };
            ...
            <>
              ...
              <div className="mt-4 pt-4 border-t border-gray-100">
                ...
                <div className="flex items-center border border-gray-200 rounded-xl overflow-hidden">
                  <input type="text" placeholder="username@upi" className="flex-1 px-4 py-3 text-sm outline-none text-gray-900" data-testid="upi-id-input" />
                  <button onClick={() => handleAppSelect({ id: 'upi', name: 'UPI' })} data-testid="upi-id-pay-btn"
                    className="px-4 py-3 bg-[#1C1D2B] text-white text-xs font-bold uppercase tracking-wide">
                    Pay
                  </button>
                </div>
              </div>
