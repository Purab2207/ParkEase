...
const PERSONA = { name: 'Arjun', previousEvents: 1, lastEvent: { name: 'Karan Aujla', bay: 'Bay B-18', month: 'Apr 2026', exitTime: '12 mins' } };
const EVENT = { title: 'RCB vs MI -- IPL 2026 Playoffs', shortTitle: 'RCB vs MI - Playoffs', venue: 'M. Chinnaswamy Stadium, Bangalore', date: 'Sun, 4 May 2026 - 7:30 PM', price: 199, totalSpots: 500, bookedSpots: 300 };
const FILL_RATE = Math.round((EVENT.bookedSpots / EVENT.totalSpots) * 100);
const SPOTS_REMAINING = EVENT.totalSpots - EVENT.bookedSpots;
...
export default function RetentionScreen() {
  return (
    <div className="min-h-screen bg-gray-50 pb-32" data-testid="retention-screen">
      <div className="max-w-md mx-auto px-4 pt-5 flex flex-col gap-4">
        <p className="text-xs text-gray-400 uppercase tracking-widest text-center">ParkEase - Re-engagement</p>
        ...
        <div className="bg-white border border-gray-200 shadow-md rounded-2xl px-4 py-3 flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-xs text-gray-500 mt-0.5">Parking spots near Chinnaswamy already 60% booked.</p>
          </div>
        </div>
        ...
        <div className="bg-gradient-to-br from-gray-800 via-gray-900 to-gray-950 rounded-2xl overflow-hidden border border-red-800/30 shadow-lg">
          <div className="relative px-5 pt-5 pb-4">
            ...
            <div className="mt-4 flex flex-col gap-2">
              <div className="flex items-center gap-2"><MapPinIcon /><span className="text-xs text-gray-300 truncate">{EVENT.venue}</span></div>
              <div className="flex items-center gap-2"><CalendarIcon /><span className="text-xs text-gray-300">{EVENT.date}</span></div>
            </div>
          </div>
        ...
        <div className="bg-white border border-gray-200 shadow-sm rounded-2xl px-4 py-4">
          <div className="flex items-center justify-between mb-1.5">
            <p className="text-xs font-semibold text-gray-700">{EVENT.bookedSpots} of {EVENT.totalSpots} spots booked</p>
            <p className="text-xs font-black text-amber-600">{FILL_RATE}% full</p>
          </div>
          <div className="w-full h-2.5 bg-gray-100 rounded-full overflow-hidden">
            <div className="h-full rounded-full" style={{ width: `${FILL_RATE}%`, background: 'linear-gradient(to right, #22c55e, #f59e0b, #ef4444)' }} />
          </div>
        ...
        <div className="bg-gray-100 border border-gray-200 rounded-xl px-4 py-3">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <span className="inline-flex items-center gap-1.5 bg-white border border-gray-200 shadow-sm rounded-lg px-3 py-1.5 text-xs font-semibold text-gray-700">
              ...
              {PERSONA.lastEvent.name} - {PERSONA.lastEvent.bay} - {PERSONA.lastEvent.month} -
        ...
        <div className="bg-blue-50 border border-blue-200 rounded-xl px-4 py-3">
          <div className="flex items-center gap-2 mb-1"><BadgeCheckIcon /><p className="text-sm font-bold text-gray-900">Welcome back, {PERSONA.name}</p></div>
          <p className="text-xs text-blue-600 mb-1">{PERSONA.previousEvents} previous event - {PERSONA.lastEvent.name}, {PERSONA.lastEvent.month}</p>
          <p className="text-xs text-gray-500">Your details are saved -- book in one tap</p>
        </div>

        {/* CTA */}
        <div className="sticky bottom-4 px-0">
          <button data-testid="rebook-cta" className="w-full bg-[#1C1D2B] text-white rounded-2xl px-6 py-4 shadow-xl active:scale-[0.98] transition-transform">
        ...
        <p className="text-xs text-gray-400 underline text-center cursor-pointer py-1">Remind me later</p>
      </div>
    </div>
  );
}