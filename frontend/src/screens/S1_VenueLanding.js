...
export default function VenueLandingScreen({
  onNavigateToBooking, onNavigateToRedirect, selectedVenue,
  totalSpots: propTotalSpots, spotsRemaining: propSpotsRemaining,
}) {
  ...
  // Prefer props from App (derived from real lot state); fall back to live WS then venue API
  const totalSpots = propTotalSpots || venue.total_spots || 180;
  const spotsRemaining = propSpotsRemaining ?? live.spotsRemaining ?? venue.spots_remaining ?? 0;
  const bookingCount = totalSpots - spotsRemaining;
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-50 font-sans" data-testid="venue-landing">
      <div className="max-w-md mx-auto min-h-[100dvh] bg-gray-50 flex flex-col px-4 py-5 gap-4 sm:shadow-2xl">
        ...
        <ScarcityCounter spotsRemaining={spotsRemaining} totalSpots={totalSpots} bookingCount={bookingCount} />
