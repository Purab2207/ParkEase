...
export default function BookingFlowScreen({ onPaymentSuccess, onBookingComplete, onNavigateBack, userPhone, isLoggedIn }) {
  ...
  }, [pendingBookingId, onPaymentSuccess, onBookingComplete]);
    ...
    <div className="min-h-[100dvh] bg-gray-50 font-sans" data-testid="booking-flow">
      <div className="max-w-md mx-auto min-h-[100dvh] bg-gray-50 flex flex-col px-4 py-5 gap-4 pb-20 sm:shadow-2xl">
        ...
        <div className="w-full flex flex-col gap-3">
          ...
          {/* Lot tabs + bay grid — only shown after user clicks CTA or has a selection */}
        ...
        <div className="w-full sticky bottom-4 mt-2">
          <button
            ...
            disabled={paymentLoading || (selectedBay && (!selectedWindow || contactPhone.length !== 10))}
            className={`w-full font-bold text-base rounded-2xl py-4 transition-all shadow-lg shadow-black/40
              ${(paymentLoading || (selectedBay && (!selectedWindow || contactPhone.length !== 10)))
