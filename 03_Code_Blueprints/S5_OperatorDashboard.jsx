...
const DashboardHeader = ({ event, status, lastUpdated, onRefresh }) => {
  const statusConfig = {
    ...
    ended:    { label: 'ENDED', classes: 'bg-gray-700 text-gray-300' },
  }[status];
    ...
    <div className="w-full flex flex-col gap-1 pt-1">
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-xs text-gray-400">Operator View</span>
        </div>
      ...
      <div className="flex items-center justify-between">
        <div className="flex flex-col">
          ...
          <span className="text-xs text-gray-500">{event.venue} · {event.date}</span>
        </div>
...
const RedirectStatusBlock = ({ active, taps, threshold, total }) => (
  <div className={`w-full rounded-2xl px-4 py-3 border flex items-center justify-between ${
    ...
    <div className="flex flex-col gap-0.5">
      <div className="flex items-center gap-2">
        <RedirectIcon />
        <span className={`text-sm font-bold ${active ? 'text-red-400' : 'text-gray-400'}`}>
    ...
    <div className={`w-3 h-3 rounded-full ${active ? 'bg-red-500 animate-pulse' : 'bg-gray-600'}`} />
  </div>
);
...
const LotCapacityBars = ({ lots }) => (
  <div className="w-full flex flex-col gap-3">
    ...
    {lots.map((lot, i) => {
      ...
      return (
        <div key={i} className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex flex-col gap-2">
          <div className="flex items-center justify-between">
            <span className="text-sm font-semibold text-white">{lot.name}</span>
            <div className="flex items-center gap-2">
              <span className={`text-sm font-bold ${isCritical ? 'text-red-400' : 'text-white'}`}>
                {lot.percent}%
              </span>
...
const AlertFeed = ({ alerts }) => {
  const config = {
    critical: { dot: 'bg-red-500',    text: 'text-red-400',    bg: 'bg-red-950 border-red-900' },
    warning:  { dot: 'bg-amber-500',  text: 'text-amber-400',  bg: 'bg-amber-950 border-amber-900' },
    success:  { dot: 'bg-green-500',  text: 'text-green-400',  bg: 'bg-gray-900 border-gray-800' },
    info:     { dot: 'bg-blue-500',   text: 'text-blue-400',   bg: 'bg-gray-900 border-gray-800' },
  };
        ...
        return (
          <div key={i} className={`w-full rounded-xl px-4 py-2.5 border flex items-start gap-3 ${c.bg}`}>
            ...
            <span className={`text-xs leading-relaxed ${
              alert.type === 'critical' ? 'text-red-300 font-semibold' :
  ...
  return (
    <div className="w-full bg-gray-900 border border-gray-800 rounded-2xl px-5 py-4 flex flex-col gap-3">
      ...
      <div className="grid grid-cols-3 gap-3 text-center">
        <div className="flex flex-col gap-0.5">
          <span className="text-xl font-black text-white">{taps}</span>
          <span className="text-xs text-gray-500">CTA taps</span>
        </div>
        <div className="flex flex-col gap-0.5">
          <span className="text-xl font-black text-green-400">~{diverted}</span>
          <span className="text-xs text-gray-500">diverted</span>
        </div>
        <div className="flex flex-col gap-0.5">
          <span className="text-xl font-black text-blue-400">{Math.round(compliance * 100)}%</span>
      ...
      <div className="flex items-center justify-between text-xs text-gray-500">
        ...
        <span className="text-white font-semibold">{industryBaseline}% MVP</span>
      </div>
...
const PDFReportButton = ({ eventName, date }) => {
  ...
  const handleDownload = () => {
    setGenerating(true);
    // PLACEHOLDER — In MVP: server-side PDF generation from dashboard data export
    setTimeout(() => {
      setGenerating(false);
      setDone(true);
  ...
  return (
    <div className="w-full flex flex-col gap-2">
      ...
      <div className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex flex-col gap-3">
        <div className="flex flex-col gap-0.5">
          <span className="text-sm font-semibold text-white">Post-event PDF report</span>
          <span className="text-xs text-gray-500">
        ...
        <p className="text-xs text-gray-600 text-center">
          {/* PLACEHOLDER — server-side PDF from dashboard data in MVP */}
          Available after event ends · {eventName} · {date}
        </p>
      </div>
    </div>
  );
...
const EventConfigSummary = ({ data }) => (
  <div className="w-full bg-gray-900 border border-gray-800 rounded-xl px-4 py-3 flex flex-col gap-2">
    ...
    {[
      ...
      <div key={i} className="flex items-center justify-between">
        ...
        <span className="text-xs text-gray-300 font-medium text-right max-w-[55%]">{value}</span>
      </div>
    ))}
  </div>
);
  ...
  return (
    <div className="min-h-[100dvh] bg-gray-950 font-sans sm:bg-gray-900">
      <div className="max-w-md mx-auto min-h-[100dvh] bg-gray-950 flex flex-col px-4 py-5 gap-5 sm:shadow-2xl">
        ...
        <PDFReportButton eventName={data.eventName} date={data.date} />
