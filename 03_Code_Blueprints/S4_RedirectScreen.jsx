...
const ParkingFullBanner = ({ venueDisplayName }) => (
  <div className="w-full bg-red-950 border border-red-700 rounded-2xl p-6 text-center flex flex-col gap-3">
...
const SurgePricingWarning = ({ surgeMultiplier }) => (
  <div className="w-full bg-amber-950 border border-amber-600 rounded-xl px-4 py-3 flex items-start gap-3">
...
const EstimatedFareDisplay = ({ fareRangeLow, fareRangeHigh, isSurgeActive }) => (
  <div className="w-full bg-gray-900 rounded-xl px-5 py-4 flex flex-col gap-1">
...
const CabProviderCard = ({ provider, availability, dropZoneLat, dropZoneLng, dropZoneName }) => {
  ...
  const getBadgeStyle = () => {
    if (availability === 'available') return "bg-green-900/80 text-green-400 text-xs rounded-full px-2 py-0.5 border border-green-800";
    if (availability === 'low') return "bg-amber-900/80 text-amber-400 text-xs rounded-full px-2 py-0.5 border border-amber-800";
    return "bg-red-900/80 text-red-400 text-xs rounded-full px-2 py-0.5 border border-red-800";
  };
  ...
  return (
    <div className={`flex flex-col items-center justify-between bg-gray-800 hover:bg-gray-700 active:scale-95 rounded-2xl p-4 gap-4 transition-all border border-gray-700 ${isUnavailable ? 'opacity-50 pointer-events-none' : 'cursor-pointer'}`} onClick={handleBooking}>
      ...
      <button 
        ...
        className="w-full bg-white text-gray-900 text-sm font-bold rounded-xl py-2.5 transition-opacity disabled:opacity-50"
      >
...
const DropZoneInfo = ({ dropZoneName }) => (
  <div className="w-full bg-gray-900 rounded-xl px-4 py-3 flex items-center gap-3">
    <div className="shrink-0 bg-gray-800 p-2 rounded-full">
      <MapPin />
    </div>
    <div className="flex flex-col min-w-0">
      <div className="text-xs text-gray-400 font-medium">Drop-off point</div>
      <div className="text-sm font-semibold text-white truncate">{dropZoneName}</div>
      <div className="text-xs text-gray-500 mt-0.5">Pre-filled in all three cab apps</div>
    </div>
  </div>
);

const AvailabilityFallbackNotice = () => (
  <div className="w-full bg-gray-800 border border-gray-600 rounded-xl px-4 py-4 flex flex-col gap-2 relative overflow-hidden">
    <div className="absolute top-0 right-0 w-2 h-full bg-red-500 rounded-r-xl"></div>
    <h3 className="text-sm font-semibold text-white">Cabs are busy right now</h3>
    ...
    <div className="mt-2 flex items-center justify-between">
      <button className="text-sm text-blue-400 font-medium underline underline-offset-2">
        View nearby pickup points
      </button>
      <span className="text-xs text-gray-500 font-mono">Refresh in 0:45</span>
    </div>
  </div>
);
...
const RedirectFooter = () => (
  <div className="w-full flex flex-col items-center gap-4 py-4 pb-8">
    ...
    <details className="group [&_summary::-webkit-details-marker]:hidden w-full max-w-sm">
      <summary className="flex items-center justify-center cursor-pointer text-xs text-gray-400 hover:text-gray-300 transition-colors">
        How is this faster?
      </summary>
      <div className="mt-3 text-xs text-gray-500 text-center bg-gray-900/50 p-3 rounded-lg border border-gray-800">
    ...
    <button className="text-sm text-gray-500 hover:text-white transition-colors flex items-center mt-2 group">
      <ArrowLeftIcon />
      <span className="underline underline-offset-4 decoration-gray-600 group-hover:decoration-gray-400 transition-colors">Back to event page</span>
    </button>
  </div>
);
