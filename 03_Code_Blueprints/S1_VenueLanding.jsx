...
const ScarcityCounter = ({ spotsRemaining, totalSpots, bookingCount }) => {
  ...
  return (
    <div className={`w-full rounded-2xl px-5 py-4 flex flex-col gap-2 border ${
      isCritical ? 'bg-red-950 border-red-800' :
      isAlmostFull ? 'bg-amber-950 border-amber-800' :
      'bg-gray-900 border-gray-800'
    }`}>
      <div className="flex items-center justify-between">
        <div className="flex items-baseline gap-1.5">
          <span className={`text-3xl font-black ${
            isCritical ? 'text-red-400' : isAlmostFull ? 'text-amber-400' : 'text-white'
          }`}>{spotsRemaining}</span>
      ...
      {isAlmostFull && (
        <p className={`text-xs font-medium ${isCritical ? 'text-red-300' : 'text-amber-300'}`}>
          {isCritical
...
const LotBreakdownList = ({ lots }) => (
  <div className="w-full flex flex-col gap-2">
    <span className="text-xs text-gray-500 uppercase tracking-widest font-semibold px-1">
      Available lots
    </span>
    {lots.map((lot, i) => (
